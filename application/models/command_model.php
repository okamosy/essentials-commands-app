...
class Command_model extends CI_Model {
	...
	private function _search_trigger($text, $release) {
		$base_sql = "
			...
			FROM cmd_release r
				...
				INNER JOIN (
					SELECT tid, MAX(version) AS version
					FROM cmd_trigger
					GROUP BY tid
				) tv ON (tv.tid = t.tid AND tv.version = t.version)
				INNER JOIN (
					SELECT rid, MAX(version) AS version
					FROM cmd_release
					GROUP BY rid
				) rv ON (rv.rid = r.rid AND rv.version = r.version)
			WHERE {$release}
				AND rtm.status =1
				AND t.status =1
				AND (%s)
	...
	private function _search_perm($text, $release) {
		$base_sql = "
			...
			INNER JOIN (
				SELECT pid, MAX(version) AS version
				FROM cmd_perm
				GROUP BY pid
			) pv ON (pv.pid = p.pid AND pv.version = p.version)
			INNER JOIN (
				SELECT tid, MAX(version) AS version
				FROM cmd_trigger
				GROUP BY tid
